<?php
	...
	if(!isset($_GET['mail'])){
		$query="SELECT * FROM message_table WHERE MSGTO='$me' AND RECEIVERFOLDER='Inbox' ORDER BY DOS DESC";
		$result=mysql_query($query) or die(mysql_error());
	//	mysql_query("delete from message_table where MSG_ID='$delete'");
	?>
		<form action="#" method="post">
		<h2 align="center">Inbox messages</h2>
			<table class="mail-form">
				<thead>
					<tr>
					<marquee behavior="alternate"><cite>This your INDOX Message</cite></marquee>
...
?>
					<tr <?=preg_match("/^[01]1$/",$rows['FLAG'])?"style='font-weight:bold; font-size:15px; color:#FFFFFF; background:#408080;
text-decoration:underline;'":"";?>>
						<th><input type="checkbox" value="" name="" /></th>
						<td style='text-align:center'><a href="./inbox.php?mail=<?=md5($rows['MSG_ID']);?>">
							<?=!empty($rows['FILE'])?'<img src="images/attach.png" alt="" />':'';?>
						</a></td>
						<td><a href="./inbox.php?mail=<?=md5($rows['MSG_ID']);?>">
					...
					<!--<td><button type="submit" class="delete_button"name="msg_delete"value="<?php echo $rows['MSG_ID'];?>"onclick="return confirm('DO YOU NEED TO DELETE THIS MESSAGE?')">DELETE</BUTTON>
						</a></td>-->
						<td> <a  class="delete_button" href="deletemessage.php?$MSG_ID=<?php echo $rows['MSG_ID']?>" ><font color="#FFFFFF">DELETE</font></a></td>
...
<table style="width:100%;border-collapse:collapse">
	<tr>
		<th colspan=3>
			<a href="REPRY.PHP?MSG_ID=<?=md5($rows['MSG_ID']);?>" ><input type="submit" name="reply" value="reply" /></a>&nbsp;&nbsp;&nbsp;&nbsp;
			<!--<input type="submit" name="forward" value="forward" />&nbsp;&nbsp;&nbsp;&nbsp;-->
			<!--<a href="FORWARD.PHP?MSG_ID=<?php echo $rows['MSG_ID']?>" ><input type="submit" name="FORWARD" value="FORWARD" /></a>&nbsp;&nbsp;&nbsp;&nbsp;-->
			<!--<input type="submit" name="forward" value="forward" />&nbsp;&nbsp;&nbsp;&nbsp;-->
